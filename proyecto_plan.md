...
- Módulo `hash_utils.py`
- Módulo `file_utils.py`
- Clase `Articulo` en `models.py`
...
3. **feature/hash-implementation** (Jordin)
4. **feature/database-hashtable** (Andrés)
5. **feature/gui-interface** (Elizabeth)
6. **feature/integration** (André)

### Proceso:
1. Cada integrante crea su branch desde `develop`
2. Realiza commits frecuentes con mensajes descriptivos
3. Al terminar, hace Pull Request a `develop`
...
proyecto-articulos/
├── src/
│   ├── models.py          # Clase Articulo (Jordin)
│   ├── hash_utils.py      # Hash FNV-1 (Jordin)
│   ├── file_utils.py      # Manejo archivos (Jordin)
│   ├── tabla_hash.py      # Tabla hash (Andrés)
│   ├── database.py        # BD persistente (Andrés)
│   ├── gui.py             # Interfaz gráfica (Elizabeth)
│   ├── controller.py      # Lógica integrada (André)
